...
337a543,548
>                         
>                         //if(node->myTag.domainID == 17  ){
>                         //printf("unpackghosts myDomain %d tag ( %d %d ) nbrtag ( %d %d) \n",home->myDomain,node->myTag.domainID,node->myTag.index,node->nbrTag[iNbr].domainID,node->nbrTag[iNbr].index);
>                         //}
>                         
>                         
350a562,576
...
> 			//printf("outPbuf %p inPBuf %p  \n",remDom->outPBuf,remDom->inPBuf);
> 
...
>             remDom = home->remoteDomainKeys[domainIdx];
>            // katso debuggerilla idom kympissä jotai ongelmaa!
>            //if ( home->myDomain==2  ) {
> 			//printf("remdom null %d  inPbuf %p inBuf %p \n",idom,remDom->inPBuf,remDom->inBuf);
>         //}
>             free(remDom->inPBuf);
>             free(remDom->outPBuf);
> //hmmm virhe tässä?
...
>  *      needs a layer of secondary ghosts which are precipitates outside
