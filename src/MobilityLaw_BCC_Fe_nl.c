...
{
        real8  alpha, beta, gamma, delta, temp;
        ...
        real8  epsilon;
        real8  b, n, ninv; 
        real8  bt, vstar, ftherm1, p, v0, vmag1; 
        real8  ftherm, dfthermdv, fdrag, dfdragdv;
        real8  fmag, dfmagdfdrag, dfmagdftherm, dfmagdv;
                   ...
                   real8 *fout, real8 *dfdv, real8 *dfdvlin, real8 *d2fdv2)
{
        ...
        real8  ratio, ratioMax;
        real8  ratioMax_low;
        real8  b, bt, vstar, vmag1, ftherm1, v0; 
        real8  taus, c0,c0p, btp, vsp, vs;
        real8  factor110; 
        real8  bzero; 
        real8  test1; 
 ...
 *
 *************************************************************************/
 /*int Mobility_BCC_Fe_nl(Home_t *home, Node_t *node, MobArgs_t *mobArgs) this is the original, got rid of mobargs (AL) */
int Mobility_BCC_Fe_nl(Home_t *home, Node_t *node)
{
        ...
        real8   fscrew[MAX_NSCREW][3], edgeb[MAX_NEDGE][3];
        real8   junctb[MAX_NBRS][3], junctdir[MAX_NBRS][3], dfndfjunct[MAX_NBRS];
        real8   edgenorm[MAX_NEDGE][3], dfndfedge[MAX_NEDGE];
        real8   fedge[MAX_NEDGE][3];
        real8   tmp;
        real8   tmp33[3][3];
        real8   correction[3];
        real8   vtmp[3];
        ...
        real8   totLen, massMult, massMatrix[3][3], vOld[3], vDiff[3], tmp3B[3];
        real8   EpsC0;
        real8   (*invdferrordv)[3], (*glideConstraints)[3];
        Param_t *param;
        Node_t  *nbrNode;


        /*invdferrordv        = mobArgs->invDragMatrix;
        glideConstraints    = mobArgs->glideConstraints;
        numGlideConstraints = &mobArgs->numGlideConstraints;*/ 

       /* Matrix33Init(invdferrordv);*/
...
/*
 *      If the node is a "fixed" node, it may not be moved, so just zero
 ...
 */
        if (param->useLabFrame) {
            ...
            Matrix33Vector3Multiply(home->rotMatrixInverse, fn, fnRot);
            Matrix33Vector3Multiply(home->rotMatrixInverse, vtest, vtestRot);
 ...
 */
            if (param->useLabFrame) {
                ...
                Matrix33Vector3Multiply(home->rotMatrixInverse,burg[i],burgRot);
                Matrix33Vector3Multiply(home->rotMatrixInverse,rt,rtRot);
            ...
            if (forceerror > ferrortol) {
                ...
                if (fabs(linmin - 0.5) < 1.0e-5) {
                    ...
                    if (numjunct == 1) {
                        ...
                        forceerror = DotProduct(ferror, ferror);
                        if (Matrix33Invert(dferrordv, invdferrordv) == 0) {
                            printf("Mobility_BCC_Fe_nl: "
                                   "Cannot invert matrix dferrordv!\n");
                            printf("iteration = %d\n", count);
                            printf("numscrews = %d\n", numscrews);
                            printf("dferrordv = \n    ");
 ...
 */
                    if (param->useLabFrame) {
                        real8 vtestRot[3];
                        Matrix33Vector3Multiply(home->rotMatrixInverse, vtest,
                                                vtestRot);
                        VECTOR_COPY(vtest, vtestRot);
                    }
 ...
 */
        if (param->useLabFrame) {
            real8 vtestRot[3];
            Matrix33Vector3Multiply(home->rotMatrix, vtest, vtestRot);
            VECTOR_COPY(vtest, vtestRot);
        }
 ...
 *
 *      Note: If the node moves outside the box, it's allowed here, the
 *      position is adjusted later in AdjustNodePosition().
 */
