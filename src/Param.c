 ...
 */
        ...
        BindVar(DPList,  (char*)"dataFileVersion", &param->dataFileVersion, V_INT,
                1, VFLAG_NULL);
        param->dataFileVersion = NODEDATA_FILE_VERSION;

        BindVar(DPList,  (char*)"numFileSegments", &param->numFileSegments, V_INT,
                1, VFLAG_NULL);
        param->numFileSegments = 1;

        BindVar(DPList,  (char*)"minCoordinates", param->minCoordinates, V_DBL, 3,
                VFLAG_NULL);

        BindVar(DPList,  (char*)"maxCoordinates", param->maxCoordinates, V_DBL, 3,
                VFLAG_NULL);

        BindVar(DPList,  (char*)"nodeCount", &param->nodeCount, V_INT, 1, VFLAG_NULL);

        BindVar(DPList,  (char*)"dataDecompType", &param->dataDecompType, V_INT, 1,
                VFLAG_NULL);
        param->dataDecompType = 2;

        BindVar(DPList,  (char*)"dataDecompGeometry", param->dataDecompGeometry,
                V_INT, 3, VFLAG_NULL);
  ...
  */
         ...
         BindVar(DPList,  (char*)"precipitateCount", &param->precipitateCount, V_INT, 1, VFLAG_NULL);
         ...
         param->dataDecompGeometry[Y] = 1;
         param->dataDecompGeometry[Z] = 1;
		 //printf("PPCNT%d:", DPList->paramCnt);
