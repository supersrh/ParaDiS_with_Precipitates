...
public:
    SYWindow(int width_hint, int height_hint,
             const char *winname, bool1 s=true, bool1 so=false,bool1 fm=false);
    virtual ~SYWindow();
    void setinterval(int i) {if (i>0) rinterval=i; }
    bool1 IsAlive() { return alive; }
    bool1 TogglePause() { pause=!pause; return pause;}
    bool1 IsPaused() { return pause; }
#ifdef _NOLOCK
    void Lock()
    {
    }
...
//    virtual void newGraph(bool1 init);
    virtual void Evolve();
...
public:
    ...
    YWindow(int w,int h,const char *n,bool1 s=true,bool1 so=true,bool1 fm=true):
